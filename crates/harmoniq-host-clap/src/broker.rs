...
impl Default for BrokerConfig {
    fn default() -> Self {
        Self {
            executable: std::env::var_os("HARMONIQ_HOST_CLAP_BROKER")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("harmoniq-host-clap-broker")),
            frames: 512,
            channels: 2,
...
impl PluginBroker {
    ...
    pub fn load_plugin(&self, path: impl AsRef<Path>) -> Result<()> {
